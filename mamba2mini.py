...
class Mamba2LMHeadModel(nn.Module):
    ...
    def generate_single(
        ...
        out, h = self(tokens, None, attention=attention, num_to_masks=num_to_masks) #self(tokens[:n_chunked], None, num_to_masks=num_to_masks)
    ...
    def forward(
        self, input_ids: LongTensor, h: list[InferenceCache] | list[None] | None = None, attention=False, num_to_masks=None
